...
- OpenClaw resolves fast mode in this order:
  ...
  6. Fallback: `off`
- Set `models.providers.<provider>.params.fastModeAllowed` to `false` to disable the shared Fast mode for every model from a provider, or set `agents.defaults.models["<provider>/<model>"].params.fastModeAllowed` for one exact model. Exact-model policy overrides the provider default. A disabled policy takes precedence over session, inline, sub-agent, automation, and per-run Fast-mode overrides; `/fast on` and `/fast auto` are rejected while that model is selected.
- Valid model-scoped `params.fastMode` / `params.fast_mode`, `params.fastModeAllowed` / `params.fast_mode_allowed`, and cutoff values are typed agent-runtime controls. They do not count as authored provider request params and do not select OpenClaw or Codex by themselves. Pin `agentRuntime.id: "openclaw"` or `agentRuntime.id: "codex"` when a recipe depends on one runtime.
...
- For direct public `anthropic/*` API-key requests, fast mode uses Anthropic native fast mode on supported models and maps other models to Anthropic service tiers (`auto` vs `standard_only`). OAuth/subscription-token requests do not receive native fast-mode or service-tier fields.
