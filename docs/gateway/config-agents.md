...
- The heartbeat object is strict. Its supported fields are `every`, `activeHours`, `model`, `session`, `target`, `directPolicy`, `to`, `accountId`, `prompt`, `skills`, `tools`, `timeoutSeconds`, `lightContext`, and `isolatedSession`.
