...
const transcriptReadCounts = vi.hoisted(() => ({ fullHistory: 0, workerDelta: 0 }));
...
vi.mock("../../session-history-worker-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../session-history-worker-runtime.js")>();
  return {
    ...actual,
    readCodexHistoryAdmissionDeltaInWorker: async (
      ...args: Parameters<typeof actual.readCodexHistoryAdmissionDeltaInWorker>
    ) => {
      transcriptReadCounts.workerDelta += 1;
      return await actual.readCodexHistoryAdmissionDeltaInWorker(...args);
    },
  };
});
...
beforeEach(() => {
  ...
  transcriptReadCounts.workerDelta = 0;
});
...
describe("Codex native transcript coverage", () => {
  it("uses one indexed delta and no full history read for an ordinary exact resume", async () => {
    ...
    // The first turn only has to record coverage; how it loads history is the
    // prompt builder's business. The resume below is what this case asserts.
    ...
    expect(transcriptReadCounts.workerDelta).toBe(1);
  ...
  it("conservatively replays a concurrent arrival after a turn with no admission receipt", async () => {
    ...
    expect(transcriptReadCounts.workerDelta).toBe(0);
  ...
  it("refreshes a generation-only stale anchor without reading full history", async () => {
    const sessionId = "coverage-stale-generation-refresh";
    const sessionFile = `agent:main:${sessionId}`;
    const storePath = path.join(tempDir, `${sessionId}.sqlite`);
    const workspaceDir = path.join(tempDir, `${sessionId}-workspace`);
    const harness = createStartedThreadHarness(async (method, requestParams) => {
    ...
    const firstParams = await createSqliteTurn({
      ...
      runId: "run-stale-generation-refresh",
      sessionFile,
      sessionId,
      storePath,
      workspaceDir,
    ...
    const secondParams = await createSqliteTurn({
      ...
      runId: "run-stale-generation-refresh:resume",
      sessionFile,
      sessionId,
      storePath,
      workspaceDir,
    ...
    expect(transcriptReadCounts.fullHistory).toBe(0);
    expect(transcriptReadCounts.workerDelta).toBe(2);
    const turnStart = harness.requests.findLast((request) => request.method === "turn/start");
    const inputText =
      (turnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]?.text ??
      "";
    expect(inputText).not.toContain("request before stale coverage");
    expect(inputText).toContain("resume after stale coverage");
  });

  it("falls back to one full history read when a stale admission cannot be refreshed", async () => {
    const sessionId = "coverage-invalid-stale-anchor-fallback";
    const sessionFile = `agent:main:${sessionId}`;
    const storePath = path.join(tempDir, `${sessionId}.sqlite`);
    const workspaceDir = path.join(tempDir, `${sessionId}-workspace`);
    const harness = createStartedThreadHarness(async (method, requestParams) => {
    ...
    const firstParams = await createSqliteTurn({
      prompt: "request before invalid stale coverage",
      runId: "run-invalid-stale-anchor-fallback",
      sessionFile,
      sessionId,
      storePath,
      workspaceDir,
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      transcriptCoverage: {
        ...
        turnStartAdmission: {
          ...coveredBinding.transcriptCoverage.turnStartAdmission,
          generation: `${coveredBinding.transcriptCoverage.turnStartAdmission.generation}:stale`,
          entryId: `${coveredBinding.transcriptCoverage.turnStartAdmission.entryId}:missing`,
        },
      },
    });
    transcriptReadCounts.fullHistory = 0;
    ...
    const secondParams = await createSqliteTurn({
      prompt: "resume after invalid stale coverage",
      runId: "run-invalid-stale-anchor-fallback:resume",
      sessionFile,
      sessionId,
      storePath,
      workspaceDir,
    ...
    const inputText =
      (turnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]?.text ??
      "";
    expect(inputText).toContain("request before invalid stale coverage");
    expect(inputText).toContain("resume after invalid stale coverage");
  });
