...
describe("maybeCompactCodexAppServerSession", () => {
  ...
  it("selects the exact bound owner across two shared keys and balances its lease", async () => {
    ...
    vi.spyOn(CodexAppServerClient, "start")
      .mockResolvedValueOnce(boundHarness.client)
      .mockResolvedValueOnce(currentHarness.client);
    ...
    expect(boundRequest).toHaveBeenCalledWith(
      "thread/compact/start",
      { threadId: "thread-1" },
      { assertCurrent: expect.any(Function) },
    );
  ...
  it("does not select a same-key successor while the exact bound owner is live-retired", async () => {
    ...
    startSpy
      .mockResolvedValueOnce(boundHarness.client)
      .mockResolvedValueOnce(successorHarness.client);
    const boundClient = await acquireSharedHarnessClient(boundHarness, "codex-bound");
    const sessionFile = await writeTestBinding({ clientId: boundClient.getInstanceId() });
    expect(retireSharedCodexAppServerClientIfCurrent(boundClient)).toEqual({
      activeLeases: 1,
  ...
  it("uses the selected owner's transport facts and preserves a same-thread successor", async () => {
    ...
    vi.spyOn(CodexAppServerClient, "start").mockResolvedValueOnce(remoteHarness.client);
  ...
  it("preserves an explicit client factory instead of selecting the bound shared owner", async () => {
    const boundHarness = createClientHarness();
    vi.spyOn(CodexAppServerClient, "start").mockResolvedValueOnce(boundHarness.client);
    const boundClient = await acquireSharedHarnessClient(boundHarness, "codex-bound");
    ...
    expect(custom.request).toHaveBeenCalledWith(
      "thread/compact/start",
      { threadId: "thread-1" },
      { assertCurrent: expect.any(Function) },
    );
...
function eraseSharedClientLifecycleMetadataForTest(client: CodexAppServerClient): void {
  // Lifecycle facts derive from start metadata, so erasing it is the only owner.
  // Read the shared state through its accessor: the global key is build-scoped.
  if (!getSharedCodexAppServerClientState().startMetadata.delete(client)) {
    throw new Error("expected shared-client start metadata");
  }
}
...
async function acquireSharedHarnessClient(
  ...
  harness.send({
    id: initialize.id,
    result: { userAgent: "openclaw/0.149.0 (Linux; compact-test)" },
  });
