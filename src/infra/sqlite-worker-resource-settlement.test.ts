...
it.for(["close-failure", "factory-failure"] as const)(
  "retains and retries native-exit receipts after %s without releasing general claims",
  async (mode, { signal }) => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "});",
        "try {",
        `  const opened = broker.open({ moduleUrl: pathToFileURL(${JSON.stringify(backend)}), databasePath: path.join(root, "store.sqlite"), input: mode });`,
        ...
        "  }",
        "  await assert.rejects(broker.close(), /cleanup failed|native Worker exit/);",
