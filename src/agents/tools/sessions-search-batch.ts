...
function buildBatchSearchPayload<Hit extends object>(params: {
  ...
  describeArchivedExclusion: (count: number) => string;
  ...
  // Archived exclusions are a scope fact of the whole batch, not of one query, so they
  // report once. Sizing and final payloads share this builder, so the reserved
  // truncation markers still bound the exact serialized bytes.
  const archivedTranscriptsExcluded = params.states.reduce(
    (total, state) => total + state.archivedTranscriptsExcluded,
    0,
  );
  const warnings = [
    ...(indexingQueries.length > 0 ? [params.indexingWarning] : []),
    ...(archivedTranscriptsExcluded > 0
      ? [params.describeArchivedExclusion(archivedTranscriptsExcluded)]
      : []),
  ];
  return {
    ...
    ...(indexingQueries.length > 0 ? { indexing: true as const, indexingQueries } : {}),
  ...
  for (let hitIndex = 0; hitIndex < params.limit; hitIndex += 1) {
    for (let queryIndex = 0; queryIndex < candidates.length; queryIndex += 1) {
      ...
      const sizingPayload = buildBatchSearchPayload({
        ...
        describeArchivedExclusion: params.describeArchivedExclusion,
