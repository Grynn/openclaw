...
describe("cron tool", () => {
  ...
  it("does not charge a wall-clock jump against the completion deadline", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
    const admission = { ok: true, enqueued: true, runId: "manual:job-clock-jump:1" };
    callGatewayMock
    ...
    const result = await tool.execute("call-clock-jump", {
      action: "run",
      jobId: "job-clock-jump",
      waitForCompletion: true,
      completionTimeoutMs: 5_000,
    });
