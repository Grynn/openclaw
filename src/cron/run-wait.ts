...
import type { CronCompletionStatus, CronDeliveryStatus, CronRunStatus } from "./types.js";

export type CronRunTerminalEntry = Record<string, unknown> & {
...
function cronCompletionStatus(value: unknown): CronCompletionStatus | undefined {
  return value === "succeeded" || value === "failed" || value === "unknown" ? value : undefined;
}

function cronDeliveryStatus(value: unknown): CronDeliveryStatus | undefined {
...
function readCronRunTerminalEntry(page: unknown): CronRunTerminalEntry | undefined {
  ...
  const {
    status,
    completionStatus: rawCompletionStatus,
    delivered: rawDelivered,
    deliveryStatus: rawDeliveryStatus,
    ...rest
  } = entry;
  const completionStatus = cronCompletionStatus(rawCompletionStatus);
  const deliveryStatus = cronDeliveryStatus(rawDeliveryStatus);
  return {
    ...rest,
    status,
    ...(completionStatus !== undefined ? { completionStatus } : {}),
    ...(typeof rawDelivered === "boolean" ? { delivered: rawDelivered } : {}),
...
export async function waitForCronRunTerminalEntry(params: {
  ...
  // Use a monotonic clock so wall-clock corrections cannot extend or
  // prematurely expire an in-flight wait.
  ...
  for (;;) {
    ...
    const remainingAfterPollMs = params.timeoutMs - Math.floor(performance.now() - startedAt);
