...
describe("SQLite active transcript event projection", () => {
  ...
  it("projects reset kept-tail and post-boundary messages without rewriting raw positions", async () => {
    ...
    const visibleRows = page.events.map(({ event }) => JSON.stringify(event));
    ...
    expect(readSessionTranscriptActiveStats(scope).eventCount).toBeGreaterThan(visibleRows.length);
