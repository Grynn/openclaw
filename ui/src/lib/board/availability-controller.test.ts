...
describe("BoardAvailabilityController", () => {
  ...
  it("reduces nine startup reads to one and retries only failed metadata", async () => {
    ...
    // Unsupported skew fails visibly: no per-session board.get storm.
    expect(legacyClient.request).not.toHaveBeenCalled();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining("requires a newer Gateway (board.metadata)"),
    );
    warn.mockRestore();
